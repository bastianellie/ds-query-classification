...
graph TD
    ...
    categories --> category_extraction[category_extraction.py]
    classifier --> category_extraction
    schema --> category_extraction
    prompts --> category_extraction
    resources --> category_extraction
    cost --> category_extraction
    category_extraction --> extract_categories_py["extract_categories.py (repo root)"]
```
...
| `src/query_classification/category_extraction.py` | Standalone entry point (spec 8): extracts a single `Category` from a free-text instruction file via one `Classifier.classify()` call, no labeled training data needed; a third orchestrator alongside `cli.py`/`experiment.py`, not a subcommand of either | `categories.py`, `classifier.py`, `schema.py`, `prompts.py`, `resources.py`, `cost.py` | `pipeline.py`, `batching.py`, `debate.py`, `multi_model.py`, `cli.py`, `experiment.py` |
...
| `extract_categories.py` (repo root) | `python extract_categories.py` script entry point; sys.path-injects `src/`, mirroring `classify.py`/`experiment.py` | `category_extraction.py` (the package module) | — |
| `tests/` | Unit tests | public package API (`query_classification`), **and** `resources.py` directly, plus the other named INV-7 exceptions — including, from spec 6, `classifier.FailureKind`/`classify_failure`/the three response-failure exception types/`_DROPPABLE_PARAMS`/`Classifier._complete`/`_attempt_completion`, from spec 5, `batching` (all functions + the private `_DATA_START`/`_DATA_END` constants), `induction._DATA_START`/`_DATA_END`, and `schema.build_batch_model`, from spec 7, `cost` (all functions/classes — not re-exported) plus `classifier._capture_usage_from_response`, and from spec 8, `category_extraction` (all functions — not re-exported), `schema.build_category_extraction_model`, and `prompts.build_category_extraction_prompt` | other internal modules/members directly beyond INV-7's named list (`categories.py`/`schema.py`/`pipeline.py` in full; anything in `classifier.py`/`cli.py` not explicitly named) |
